...
**Кэш и обновления:** интерфейс и скрипты отдаются **сначала из сети** (сервис-воркер обновляет кэш в фоне), чтобы после деплоя не «залипала» старая вёрстка и логика. `localStorage` хранит только данные партии и настройки музыки, а не разметку — если после обновления сайта всё ещё странное поведение, в DevTools → Application: **Unregister** service worker и **Clear site data**, затем обновите страницу.
